//////////////////////////////
/**
 * KanjiPreloader.ts
 *
 * Quản lý tải trước dữ liệu nét chữ song song.
 * Tự động nhận diện chữ thiếu nét/chữ mới -> Fetch CDN -> Caching tự động qua AsyncStorage.
 */

import AsyncStorage from '@react-native-async-storage/async-storage';
import { getDb } from './db';
export const memoryCache = new Map<string, string[]>();

const pendingCdnFetches = new Map<string, Promise<{ paths: string[]; source: 'cdn' | 'none' }>>();
const CDN_BASE = 'https://cdn.jsdelivr.net/gh/kanjivg/kanjivg@master/kanji/';
const ASYNC_PREFIX = 'kanji_stroke_v1:';

// ── Helpers ────────────────────────────────────────────────────────────────

function normalizeKanjiInput(input: string): { char: string; wasUnicodeId: boolean } {
  if (!input) return { char: input, wasUnicodeId: false };

  const normalized = input.normalize('NFC');
  if (normalized !== input) {
    console.warn(
      `[KanjiPreloader] 🔧 NFC normalize: "${input}" (hex ${input.codePointAt(0)?.toString(16)}) ` +
      `→ "${normalized}" (hex ${normalized.codePointAt(0)?.toString(16)})`
    );
  }
  input = normalized;

  const m = input.trim().match(/^u\+?([0-9a-fA-F]{4,6})$/i);
  if (m) {
    try {
      const char = String.fromCodePoint(parseInt(m[1], 16));
      console.warn(
        `[KanjiPreloader] ⚠️ PHÁT HIỆN "kanji" bị truyền dạng ID: "${input}" → tự sửa thành ký tự thật: "${char}"`
      );
      return { char, wasUnicodeId: true };
    } catch {
      return { char: input, wasUnicodeId: false };
    }
  }

  // Trường hợp input dài bất thường (vd id ghép "器_器_2") → không phải ký tự đơn
  if (input.length > 2 && /_/.test(input)) {
    console.warn(
      `[KanjiPreloader] ⚠️ "kanji" trông giống ID ghép (có dấu "_"): "${input}" — không phải 1 ký tự Hán đơn lẻ!`
    );
  }

  return { char: input, wasUnicodeId: false };
}

function toHexId(char: string): string {
  return char.codePointAt(0)!.toString(16).toLowerCase().padStart(5, '0');
}

function parseSvgPaths(rawSvg: string): string[] | null {
  const clean = rawSvg.replace(/<g[^>]*id="kvg:StrokeNumbers[\s\S]*$/, '');
  const paths: string[] = [];
  const re = /<path[^>]*\bd="([^"]+)"/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(clean)) !== null) {
    const d = m[1].trim();
    if (d) paths.push(d);
  }
  return paths.length > 0 ? paths : null;
}

async function getStrokesFromDb(kanji: string): Promise<string[] | null> {
  try {
    // 🔍 LOG HEX ĐẦY ĐỦ — không chỉ ký tự đầu, mà toàn bộ codepoint trong chuỗi
    const hexDump = [...kanji].map(c => c.codePointAt(0)!.toString(16));
    console.log(`[KanjiPreloader] getStrokesFromDb nhận kanji="${kanji}" length=${kanji.length} hexDump=[${hexDump.join(',')}]`);

    const db = await getDb('kanji');
    if (!db) return null;

    const row = await db.getFirstAsync<{ paths: string }>(
      'SELECT paths FROM kanji_strokes WHERE kanji = ?',
      [kanji],
    );
    console.log(`[KanjiPreloader] 🔍 Query kanji_strokes WHERE kanji = "${kanji}" (hex=[${hexDump.join(',')}]) → row =`, row ? 'FOUND' : 'NULL');

    if (!row?.paths) return null;
    const parsed: string[] = JSON.parse(row.paths);
    return Array.isArray(parsed) && parsed.length > 0 ? parsed : null;
  } catch (err) {
    console.warn(`[KanjiPreloader] ❌ lỗi đọc DB cho "${kanji}":`, err);
    return null;
  }
}

// async function getStrokesFromDb(kanji: string): Promise<string[] | null> {
//   try {
//     const db = await getDb('kanji');
//     if (!db) return null;

//     const row = await db.getFirstAsync<{ paths: string }>(
//       'SELECT paths FROM kanji_strokes WHERE kanji = ?',
//       [kanji],
//     );
//     if (!row?.paths) return null;

//     const parsed: string[] = JSON.parse(row.paths);
//     return Array.isArray(parsed) && parsed.length > 0 ? parsed : null;
//   } catch (err) {
//     console.warn(`[KanjiPreloader] lỗi đọc DB cho "${kanji}":`, err);
//     return null;
//   }
// }

function isStrokeDataInvalid(kanji: string, paths: string[] | undefined | null): boolean {
  if (!paths || paths.length === 0) return true;

  const count = paths.length;
  const code = kanji.codePointAt(0) || 0;

  if (code < 0x4e00 || code > 0x9faf) return false;

  const ultraSimple = new Set([
    '一', '二', '三', '亠', '人', '入', '八', '几', '凵', '刀', '力', '勹', '匕', '匚', '十', '卜', '卩', '厂', '厶', '又', '口', '囗', '土', '士', '夂', '夕', '大', '女', '子', '宀', '寸', '小', '尢', '尸', '屮', '山', '川', '工', '己', '巾', '干', '幺', '广', '廴', '廾', '弋', '弓', '彐', '彡', '彳', '才', '万', '丈', '与', '丑', '专', '丸', '也', '于', '亡', '及', '久', '个'
  ]);
  if (ultraSimple.has(kanji)) return false;
  if (count < 1 || count > 30) return true;
  return false;
}

async function fetchFromCdnDeduped(kanji: string): Promise<{ paths: string[]; source: 'cdn' | 'none' }> {
  if (pendingCdnFetches.has(kanji)) {
    console.log(`[KanjiPreloader] ⏳ Đã có fetch CDN đang chạy cho "${kanji}", dùng chung kết quả`);
    return pendingCdnFetches.get(kanji)!;
  }

  const promise = (async () => {
    const hexId = toHexId(kanji);
    const url = `${CDN_BASE}${hexId}.svg`;
    //console.log(`[KanjiPreloader] → Thử CDN: ${url}`);
    try {
      const res = await fetch(url);
      //console.log(`[KanjiPreloader] CDN response status = ${res.status} cho "${kanji}"`);
      const text = await res.text();
      const fetched = parseSvgPaths(text);
      console.log(`[KanjiPreloader] parseSvgPaths → ${fetched?.length ?? 0} path(s) cho "${kanji}"`);
      if (fetched && fetched.length > 0) {
        AsyncStorage.setItem(ASYNC_PREFIX + kanji, JSON.stringify(fetched)).catch(() => {});
        memoryCache.set(kanji, fetched);
        return { paths: fetched, source: 'cdn' as const };
      }
    } catch (err) {
      console.warn(`[KanjiPreloader] ❌ Fetch CDN thất bại cho "${kanji}":`, err);
    }
    return { paths: [], source: 'none' as const };
  })();

  pendingCdnFetches.set(kanji, promise);
  try {
    return await promise;
  } finally {
    pendingCdnFetches.delete(kanji);
  }
}

export async function loadStrokePaths(
  kanjiRaw: string,
  forceRefresh: boolean = false,
): Promise<{ paths: string[]; source: 'local' | 'cache' | 'cdn' | 'none' }> {

  // ── 🔍 LOG ĐẦU VÀO — quan trọng nhất để xác định bug ──────────────────────
  const { char: kanji, wasUnicodeId } = normalizeKanjiInput(kanjiRaw);
  console.log(
    `[KanjiPreloader] ═══ loadStrokePaths("${kanjiRaw}") ═══\n` +
    `  → sau chuẩn hóa: "${kanji}" (đã sửa từ id? ${wasUnicodeId})\n` +
    `  → codePointAt(0): 0x${kanji.codePointAt(0)?.toString(16)}\n` +
    `  → độ dài chuỗi: ${kanji.length}\n` +
    `  → toHexId sẽ dùng: ${toHexId(kanji)}`
  );

  if (forceRefresh) {
    const hexId = toHexId(kanji);
    const url = `${CDN_BASE}${hexId}.svg`;
    console.log(`[KanjiPreloader] forceRefresh → fetch: ${url}`);
    try {
      const res = await fetch(url);
      console.log(`[KanjiPreloader] CDN status = ${res.status}`);
      const text = await res.text();
      const fetched = parseSvgPaths(text);
      console.log(`[KanjiPreloader] parseSvgPaths → ${fetched?.length ?? 0} path(s)`);
      if (fetched && fetched.length > 0) {
        await AsyncStorage.setItem(ASYNC_PREFIX + kanji, JSON.stringify(fetched));
        memoryCache.set(kanji, fetched);
        return { paths: fetched, source: 'cdn' };
      }
    } catch (err) {
      console.warn(`[KanjiPreloader] Force refresh lỗi cho "${kanji}":`, err);
    }
  }

  // 1. SQLite local
  const fromDb = await getStrokesFromDb(kanji);
  if (fromDb && !isStrokeDataInvalid(kanji, fromDb)) {
    console.log(`[KanjiPreloader] ✅ Dùng nguồn LOCAL cho "${kanji}" (${fromDb.length} nét)`);
    memoryCache.set(kanji, fromDb);
    return { paths: fromDb, source: 'local' };
  }

  // 2. Memory cache
  if (memoryCache.has(kanji)) {
    const cachedMemory = memoryCache.get(kanji)!;
    if (!isStrokeDataInvalid(kanji, cachedMemory)) {
      console.log(`[KanjiPreloader] ✅ Dùng MEMORY CACHE cho "${kanji}"`);
      return { paths: cachedMemory, source: 'cache' };
    }
  }

  // 3. AsyncStorage
  try {
    const stored = await AsyncStorage.getItem(ASYNC_PREFIX + kanji);
    if (stored) {
      const parsed: string[] = JSON.parse(stored);
      if (parsed.length > 0 && !isStrokeDataInvalid(kanji, parsed)) {
        console.log(`[KanjiPreloader] ✅ Dùng ASYNC STORAGE cache cho "${kanji}"`);
        memoryCache.set(kanji, parsed);
        return { paths: parsed, source: 'cache' };
      }
    } else {
      console.log(`[KanjiPreloader] AsyncStorage: chưa có cache cho "${kanji}"`);
    }
  } catch (err) {
    console.warn(`[KanjiPreloader] Lỗi đọc AsyncStorage cho "${kanji}":`, err);
  }

  // 4. CDN cứu hộ
  const cdnResult = await fetchFromCdnDeduped(kanji);
  if (cdnResult.paths.length > 0) {
    return { paths: cdnResult.paths, source: 'cdn' };
  }

  if (fromDb && fromDb.length > 0) {
    console.log(`[KanjiPreloader] Fallback: dùng DB dù nghi ngờ thiếu nét cho "${kanji}"`);
    return { paths: fromDb, source: 'local' };
  }

  console.warn(`[KanjiPreloader] 🚫 KHÔNG TÌM ĐƯỢC NÉT VẼ NÀO cho "${kanji}" (raw input: "${kanjiRaw}")`);
  return { paths: [], source: 'none' };
}

// ── Preloader Class ────────────────────────────────────────────────────────
class KanjiPreloader {
  private preloadQueue: Set<string> = new Set();
  private isPreloading = false;

  warmup(): void {}

  async preloadKanjiBatch(kanjiList: string[]): Promise<void> {
    return this.preloadStrokePaths(kanjiList);
  }

  async preloadStrokePaths(kanjiList: string[]): Promise<void> {
    for (const k of kanjiList) {
      if (!memoryCache.has(k)) {
        this.preloadQueue.add(k);
      }
    }
    this.processQueue();
  }
  
  preloadVocabBatch(vocabList: string[]): void {
    const kanjiChars: string[] = [];
    vocabList.forEach(word => {
      for (const char of word) {
        if (/[\u3400-\u9fff]/.test(char)) kanjiChars.push(char);
      }
    });
    if (kanjiChars.length > 0) this.preloadStrokePaths(kanjiChars);
  }

  preloadGrammarBatch(grammarList: string[]): void {}
  preloadSentence(sentenceId: string): void {}

  async preloadSurroundingKanji(
    kanjiChars: string[],
    currentIndex: number,
    windowSize = 3,
  ): Promise<void> {
    const start = Math.max(0, currentIndex - 1);
    const end   = Math.min(kanjiChars.length - 1, currentIndex + windowSize);
    const toLoad = kanjiChars.slice(start, end + 1);
    this.preloadStrokePaths(toLoad);
  }

  private async processQueue(): Promise<void> {
    if (this.isPreloading || this.preloadQueue.size === 0) return;
    this.isPreloading = true;

    for (const kanji of this.preloadQueue) {
      this.preloadQueue.delete(kanji);
      await loadStrokePaths(kanji);
      await new Promise(r => setTimeout(r, 40));
    }

    this.isPreloading = false;
    if (this.preloadQueue.size > 0) this.processQueue();
  }

  isReady(kanji: string): boolean {
    return memoryCache.has(kanji);
  }

  getMissingFromLocal(kanjiList: string[]): string[] {
    return kanjiList.filter(k => !memoryCache.has(k));
  }

  async clearCache(kanji: string): Promise<void> {
    memoryCache.delete(kanji);
    await AsyncStorage.removeItem(ASYNC_PREFIX + kanji);
  }

  async clearAllCache(): Promise<void> {
    memoryCache.clear();
    const keys = await AsyncStorage.getAllKeys();
    const strokeKeys = keys.filter(k => k.startsWith(ASYNC_PREFIX));
    await AsyncStorage.multiRemove(strokeKeys);
  }
}

export const preloader = new KanjiPreloader();
export default preloader;