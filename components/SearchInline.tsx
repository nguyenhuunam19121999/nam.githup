...
//                 } else {
//                   isDirect = searchChars.includes(k.kanji);
//                   match = isDirect ||
...
//                   if (isDirect) directMatches.push(k);
//                   else indirectMatches.push(k);
//                 }
//               }

//               const sortedDirect = [...directMatches].sort((a, b) =>
//                 searchChars.indexOf(a.kanji) - searchChars.indexOf(b.kanji)
//               );

//               const foundInChunk = [...sortedDirect, ...indirectMatches];

//               if (foundInChunk.length > 0) {
//                 allFoundKanji.push(...foundInChunk);
//                 updateUI();
    ...
    (targetQuery: string, targetTab: SearchType) => {
      ...
      InteractionManager.runAfterInteractions(() => {
        ...
        const runKanjiStream = async () => {
          ...
          //       let isDirect = false;
          //       if (isVietnamese) {
          //         match = k.hanviet?.some(h => normalizeString(h).includes(nq)) ||
          //         k.meanings_vi?.some((m: string) => normalizeString(m).includes(nq));
          //       } else {
          //         isDirect = searchChars.includes(k.kanji);
          //         match = isDirect ||
          //           k.readings?.kunyomi?.some((r: string) => r.includes(targetQuery)) ||
          //           k.readings?.onyomi?.some((r: string) => r.includes(targetQuery));
          //       }
          ...
          // Gọi SQLite trực tiếp thay vì duyệt kanjiByLevel
          const kanjiResults = await searchKanji(targetQuery);
            if (token !== searchTokenRef.current) return;
            ...
            for (const k of kanjiResults) {
              if (seenKanji.has(k.kanji)) continue;
              seenKanji.set(k.kanji, true);
              const isDirect = searchChars.includes(k.kanji);
              if (isDirect) directMatches.push(k);
              else indirectMatches.push(k);
            }
