...
const PICK_INDUSTRIES = Object.values(INDUSTRY_INFO || {}).map(
  (ind: any, idx: number) => ({
...
interface HomeSuggestionsProps {
  onSelectSuggestion: (
    query: string,
    tab: "vocab" | "sentence" | "grammar",
  ) => void;
}

async function getTopLevelFromHistory(
  history: { text: string }[],
  allVocab: any[],
): Promise<string> {
  ...
  for (const item of history) {
    const found = allVocab.find(
      (v: any) => v.kanji === item.text || v.hiragana === item.text,
    );
  ...
  const KEY_MAJOR = scopedKey("user_custom_major");
    ...
    try {
      ...
      if (!savedLevel) {
        try {
          const raw = await AsyncStorage.getItem(
            scopedKey("search_history_vocab"),
          );
          if (raw) {
      ...
      if (savedMajor) {
        ...
        if (savedLevel) {
          words = industryPool.filter(
            (v) => v.level === currentLevel || v.Level === currentLevel,
          );
        } else {
          words = industryPool;
        }
      ...
      if (words.length === 0 && savedLevel) {
        ...
        words = [...(filtered.length > 0 ? filtered : allVocab)]
      ...
      for (const v of words) {
        ...
        if (v.example && v.example.trim()) {
          ...
          if (!seenSentenceTexts.has(finalJp1)) {
            ...
            result.push({
              ...
              wordKey: uniqueWordKey,
              primary:
                finalJp1.length > 44 ? finalJp1.slice(0, 44) + "…" : finalJp1,
              meaning:
                finalVi1.length > 52 ? finalVi1.slice(0, 52) + "…" : finalVi1,
        ...
        if (validSentences.length === 0) {
          const backupSent = EXAMPLE_SENTENCES.find((s) =>
            s.jp.includes(uniqueWordKey),
          );
          if (backupSent) {
            ...
            if (!seenSentenceTexts.has(finalJp2)) {
              ...
              result.push({
                ...
                wordKey: uniqueWordKey,
                primary:
                  finalJp2.length > 44 ? finalJp2.slice(0, 44) + "…" : finalJp2,
                meaning:
                  (backupSent.vi || "").length > 52
                    ? (backupSent.vi || "").slice(0, 52) + "…"
                    : backupSent.vi || "",
                level: v.level || v.Level,
                searchQuery: uniqueWordKey,
                searchTab: "sentence",
        ...
        for (const textSentence of validSentences) {
          const gram = allGrammar.find(
            (g) => g.pattern && textSentence.includes(g.pattern),
          );
  ...
  if (loading) {
    return (
      <View style={styles.loadingRow}>
        ...
        <Text style={[styles.loadingTxt, { color: c.mutedForeground }]}>
          Đang tải gợi ý...
        </Text>
      </View>
    );
          ...
          {JLPT_LEVELS.map((lv) => (
            <TouchableOpacity
              ...
              style={[
                ...
                surveyLevel === lv.label && {
                  backgroundColor: c.primary,
                  borderColor: c.primary,
                },
              ]}
              onPress={() =>
                setSurveyLevel(surveyLevel === lv.label ? "" : lv.label)
              }
            >
              <Text
                style={[
        ...
        {PICK_INDUSTRIES.length > 0 && (
          <>
            <Text
              style={[
                styles.pickerLabel,
                { color: c.mutedForeground, marginTop: 14 },
              ]}
            >
              Ngành học
            </Text>
              ...
              <View style={{ flexDirection: "row", gap: 6, paddingRight: 16 }}>
                {PICK_INDUSTRIES.map((ind) => (
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setSurveyIndustry(
                        surveyIndustry === ind.key ? "" : ind.key,
                      )
                    }
                  >
                    <Text
  ...
  return (
    <View style={styles.wrap}>
      <View style={styles.titleRow}>
        <Text style={[styles.titleTxt, { color: c.mutedForeground }]}>
          ✨ Gợi ý tìm kiếm hôm nay
        </Text>
        <Text style={[styles.countTxt, { color: c.mutedForeground }]}>
          {cards.length} thẻ
        </Text>
      </View>
      ...
      >
        ...
        >
          {cards.map((card) => (
            <View
              key={card.id}
              style={{ width: containerWidth, paddingHorizontal: 4 }}
            >
              <TouchableOpacity
                style={[
                  styles.cardSearchStyle,
                  { borderColor: c.border },
                  cardBg(card.kind),
                ]}
                activeOpacity={0.82}
                onPress={() =>
                  onSelectSuggestion(card.searchQuery, card.searchTab)
                }
              >
                <Text style={[styles.bgSearchIcon, { color: c.text }]}>🔍</Text>
                <View style={[styles.kindBadge, kindBadgeStyle(card.kind)]}>
                  <Text style={styles.kindBadgeTxt}>
                    {kindLabel(card.kind)}
                  </Text>
                </View>
                ...
                {card.level && (
                  <View
                    style={[
                      ...
                      {
                        backgroundColor:
                          (LEVEL_COLORS[card.level] ?? "#64748b") + "22",
                      },
                    ]}
                  >
                    <Text
                      style={[
                        styles.levelTxt,
                        { color: LEVEL_COLORS[card.level] ?? "#64748b" },
                      ]}
                    >
                      {card.level}
                    </Text>
                ...
                <Text
                  style={[styles.primary, primaryStyle(card.kind)]}
                  numberOfLines={2}
                >
                  {card.primary}
                </Text>
                {!!card.reading && (
                  <Text
                    style={[styles.reading, { color: c.mutedForeground }]}
                    numberOfLines={1}
                  >
                    {card.reading}
                  </Text>
                ...
                <Text
                  style={[styles.meaning, { color: c.mutedForeground }]}
                  numberOfLines={2}
                >
                  {card.meaning}
                </Text>
                ...
                {card.kind !== "vocab" && (
                  <View style={styles.wordRefRow}>
                    ...
                    <Text
                      style={[styles.wordRef, { color: c.mutedForeground }]}
                    >
                      {card.wordKey}
                    </Text>
                  </View>
                ...
                <View style={styles.searchAction}>
                  <Text style={[styles.searchActionTxt, { color: c.primary }]}>
                    Chạm để tra cứu ➔
                  </Text>
                </View>
              </TouchableOpacity>
...
function primaryStyle(kind: CardKind) {
  if (kind === "vocab")
    return { color: "#004370", fontSize: 26, fontWeight: "800" as const };
  if (kind === "sentence")
    return { color: "#1e3a5f", fontSize: 15, fontWeight: "700" as const };
